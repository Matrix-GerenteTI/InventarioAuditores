...
class RevisionInventarios  extends PrepareExcel
{
    ...
    public function generaReporte( $mes, $anio )
    {
        ...
        $auditores = [
            ['auditor' => 'C.P. Felipe Rosales Rosales','ID' => 1],
            ['auditor' => 'C.P. Feliciano Coutino de la Torre', 'ID' => 2]
        ...
        $sucursalInventario = [];
        $filaUbicacion = 9;
        foreach ( $auditores as  $sucursal) {
            $sucursalInventario[ $sucursal['ID'] ]['auditor'] = $sucursal['auditor'];
        ...
        $this->libro->getActiveSheet()->setCellValue("A8", "AUDITOR");
        ...
        foreach ($sucursalInventario as $sucursal) {
            $this->libro->getActiveSheet()->setCellValue("A".$sucursal['fila'], $sucursal['auditor']);
            ...
            for ($i=1; $i <= $cantDiasMes  ; $i++) { 
                ...
                $inventariosOmitidosSucursales = $modeloInventario->getSucursalesInventariosOmitidos( $anio, $mes, $i, $sucursal['auditor'] );
            ...
            foreach ($auditoresInventario as $auditor) {
...
$mails = array('gerenteti@matrix.com.mx','ti@matrix.com.mx','dispersion@matrix.com.mx','corporativo@matrix.com.mx','cavim@matrix.com.mx','admonrh@matrix.com.mx','rhmatrix2019@gmail.com');
