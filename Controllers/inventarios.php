...
require_once $_SERVER['DOCUMENT_ROOT']."/InventarioAuditores/Controllers/reporteInventarioInmediato.php";
...
class InventariosController
{
    ...
    public function getInventarioProgramadoSucursal( $sucursal ) //carga al seleccionar sucursal en login
    {
        // // var_dump(__FUNCTION__. "deteniendo proceso");
    ...
    {
        // // var_dump(__FUNCTION__. "deteniendo proceso"); //Funcion que genera el inventario aleatorio, no guarda, solo muestra, es la primera pantalla
                                                      // despues de haber dado clic en continuar
    ...
    {
        // var_dump(__FUNCTION__. "deteniendo procesoooooo");
        $inventario = new Inventario();
    ...
    /*-------------------------------FUNCION ESTRELLA QUE GENERA EL INVENTARIO-----------------------*/
    ...
    public function RegistrarInventarioGeneralInmediato($productos){
        ...
        foreach ($productos as $producto_ind) {
            extract($producto_ind);
            if ($stkIngresado === true) {
                array_push($productos_marcados, $producto_ind);
            }
        }
        $reporte = new ReporteInventarioInmediato;
        $reporte->generarReporte($productos_marcados);
...
if ( isset($_GET['opc']))  {
   ...
   if(!isset($_POST['usuario'])){
    ...
    $inventarioContent = $inventarioData['inventario'];
        ...
        //$auditor = $inventarioData['auditor'];
    ...
    if ($tipoOperacion > 0) {
        ...
        foreach ($inventarioContent as $i => $item) {
            extract( $item );
            $registros .= "('','$CODIGOARTICULO','$DESCRIPCION','$FAMILIA','$SUBFAMILIA','$CANTIDAD',NULL,NULL,$EXISTENCIA,'$tipo' ,'$sucursal',now(),0 ,NULL,1,NULL,'$auditor')";
