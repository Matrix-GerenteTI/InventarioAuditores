...
require_once $_SERVER['DOCUMENT_ROOT']."/InventarioAuditores/Config/Database.php";
class Inventario
{
    ...
    public function getInventarioRealizado( $data)
    {
        ...
        where fechaCaptura like '$fecha%' AND tipo = $tipo AND sucursal_id = $sucursal AND familia = '$familia'  and subfamilia = '$subfamilia' ";
    ...
    public function getInventariosSucursal( $data)
    {
        ...
        $queryInventario = "SELECT inventarios.*, csucursal.descripcion as sucursal 
                                ...
                                where auditor IS NOT NULL AND date(fechaCaptura) = '$fecha' AND sucursal_id = $sucursal
    ...
    public function getArticulosNoInventariados( $sucursal , $filtraFamilia = null )
    {
        ...
        FROM CFG_ARTICULOS  
        INNER JOIN REF_ARTXALMACEN ON REF_ARTXALMACEN.FK1MCFG_ARTICULOS = CFG_ARTICULOS.ID
        WHERE REF_ARTXALMACEN.FK1MCFG_ALMACENES = $sucursal AND CFG_ARTICULOS.FAMILIA  IN $familiaUsuario ORDER BY SUBFAMILIA,FAMILIA DESC ";*/
    ...
    public function registraInventario($data)
    {
        ...
        if(isset($usuario))
            $queryRegistraInventario = "INSERT INTO inventarios VALUES ('','$codigo','$descripcion','$familia','$subfamilia','$stkIngresado',$fisico2,$fisico3,'$stock','$tipo','$sucursal', now(),NULL,'$usuario',1,NULL)";
        else
            $queryRegistraInventario = "INSERT INTO inventarios VALUES ('','$codigo','$descripcion','$familia','$subfamilia','$stkIngresado',$fisico2,$fisico3,'$stock','$tipo','$sucursal', now(),NULL,NULL,1,NULL)";
    ...
    public function RegistraInventarioGeneral( $data , $idHistorialInventario = -1)
    {
        ...
        foreach ($data as $i => $producto) {
            ...
            if(isset($usuario) || ( session_status() == PHP_SESSION_ACTIVE && sizeof( $_SESSION) > 0 )  ){
                ...
                $queryRegistraInventario .="('','$codigo','$descripcion','$familia','$subfamilia','$stkIngresado',$fisico2,$fisico3,'$stock','$tipo','$sucursal', now(),'$tiempoTranscurrido','$usuario',1,NULL,'$auditor')";
            }
            else{
                $usuario = isset( $usuario) ? $usuario : $_SESSION['usuario'];
                $queryRegistraInventario .="('','$codigo','$descripcion','$familia','$subfamilia','$stkIngresado',$fisico2,$fisico3,'$stock','$tipo','$sucursal', now(),'$tiempoTranscurrido','$usuario',1,".( $idHistorialInventario != -1 ? $idHistorialInventario : 'NULL') . ",'$auditor')";
            }
